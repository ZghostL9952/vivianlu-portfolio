// Adapted from dashersw/liquid-glass-js (MIT); see LIQUID_GLASS_LICENSE.
...
export default class Container {
    ...
    html2canvas(document.body, {
      ...
      ignoreElements: function (element) {
        ...
        return (
          ...
          element.classList.contains('tedx-note')
        )
      }
    })
      .then(snapshot => {
        ...
        waitingContainers.forEach(container => {
          if (!container.destroyed && !container.webglInitialized) {
            container.initWebGL()
          }
        })
      })
  ...
  startRenderLoop() {
    const render = () => {
      if (this.destroyed || !this.gl_refs.gl) return
  ...
  destroy() {
    ...
    const instanceIndex = Container.instances.indexOf(this)
    if (instanceIndex !== -1) Container.instances.splice(instanceIndex, 1)
    this.gl_refs = {}
  }
